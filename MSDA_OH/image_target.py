...
def cal_acc(clean_loader, netBB, netMF, netMB, netMC, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in data_bar:
            ...
            clean_images1 = clean_images.cuda()
    ...
    if flag:
        matrix = confusion_matrix(all_label_clean, torch.squeeze(predict_clean).float())
...
def train_target(args):
    ...
    modelpath = args.stored_dir_src_main + '/model.pt'   
    ...
    modelpath = args.stored_dir_src_st + '/model.pt'   
    ...
    weights = torch.unsqueeze(torch.tensor([1]*10), 0).cuda()
    iter_num = 0
    acc_init = 0
    ...
    for step_i in train_data_bar:
        ...
        train_data_bar.set_description("Train : Step:{}, MB_class_loss:{:.4f}, St_Loss:{:.4f}".format(iter_num, loss, SB_loss))
        if (iter_num % 100) == 0 or iter_num == max_iter:
            ...
            MB_log= cal_acc(dset_loaders['test'], netBB, netMF, netMB, netMC, False)
            args.logger.info('Step: [{}/{}], Adapt_Acc.:{:.4f}'.format(iter_num,
            max_iter, MB_log[0]))

            if MB_log[0] >= acc_init:
                acc_init = MB_log[0]
                args.logger.info("Update Best Adapt Acc: {}".format(acc_init))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size_bg', type=int, default=32, help="batch_size")
    ...
    args.output_dir_src = osp.join('Ckpt_Ad_nrc_unf{}'.format(args.lr), args.dset, names[args.t].upper())
    if not osp.exists(args.output_dir_src):
        os.system('mkdir -p ' + args.output_dir_src)
    if not osp.exists(args.output_dir_src):
        os.mkdir(args.output_dir_src)
    args.stored_dir_src_main = osp.join('Checkpoint_main', args.dset, names[args.t].upper())
    args.stored_dir_src_st = osp.join('Checkpoint_Sticker', args.dset, names[args.t].upper())
