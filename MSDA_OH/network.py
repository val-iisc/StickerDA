...
class ResBase_Layer4(nn.Module):
    def __init__(self, res_name):
        super(ResBase_Layer4, self).__init__()
        model_resnet = res_dict[res_name](pretrained=True)
...
class feat_classifier(nn.Module):
    def __init__(self, class_num, bottleneck_dim=256, sticker_class = 11, main = True, type="linear"):
        ...
        if type == 'wn':
            if self.main:
                ...
                self.fc = weightNorm(nn.Linear(bottleneck_dim, sticker_class), name="weight")
                self.fc.apply(init_weights)
        else:
