...
def cal_acc_oda(args, clean_loader, sticker_loader, bg_loader, netBB, netF, netB, netC, weights):
    ...
    with torch.no_grad():
        ...
        for i in data_bar:
            ...
            mixed_up, mixed_up_labels = mixup_batch(clean_images, mixup_stickers[:args.batch_size]).cuda(), mixup_labels.cuda()[:len(clean_images)]
            ...
            loss = CrossEntropyLabelSmooth(num_classes=args.sticker_num, weights=weights, epsilon=args.smooth, reduction=True)(outputs, labels)
            
            accumulated_loss.update(loss.item())
            data_bar.set_description("{} : Step:{}, Class_loss:{:.4f}".format(args.model, i, loss))
            if start_test:
    ...
    return accumulated_loss.avg, acc[:-1], np.mean(acc[:-1]), ood_acc, np.mean(acc) 
...
def train_source(args):
    ...
    for step_i in train_data_bar:
        ...
        clean_images = clean_images.cuda()
        mixed_up_images, mixed_up_labels = mixup_batch(clean_images, mixup_stickers[:args.batch_size]).cuda(), mixup_labels.cuda()[:len(clean_images)]
        ...
        if ((iter_num) % 500 == 0) or (iter_num == max_iter):
            ...
            # Validation Logs
            args.logger.info("Val: Loss:{} , Per_Class_Acc:{} , Class_mean_Acc:{}, OOD_Acc:{}, Mean_Acc: {}".format(val_loss_acc[0],
            val_loss_acc[1], val_loss_acc[2], val_loss_acc[3], val_loss_acc[4]))
            
            # Test Logs
            args.logger.info("Test: Loss:{}, Per_Class_Acc:{} , Class_mean_Acc:{}, OOD_Acc:{}, Mean_Acc: {}".format(test_loss_acc[0],
            test_loss_acc[1], test_loss_acc[2], test_loss_acc[3], test_loss_acc[4]))

            
            if val_loss_acc[4] >= acc_init:
                acc_init = val_loss_acc[4]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--s', type=int, default=[0,1,3], nargs = 3, help="source")
    ...
    np.random.seed(SEED)
    random.seed(SEED)
    args.logger = get_logger('__train__', 'logs_{}_St.txt'.format(names[args.t]))
    ...
    args.output_dir_src = osp.join('Checkpoint_Sticker', args.dset, names[args.t].upper())
    if not osp.exists(args.output_dir_src):
        os.system('mkdir -p ' + args.output_dir_src)
    if not osp.exists(args.output_dir_src):
        os.mkdir(args.output_dir_src)
    args.stored_model_dir_src = osp.join('Checkpoint_main', args.dset, names[args.t].upper())
